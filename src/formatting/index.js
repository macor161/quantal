...
const defaultFormatSourceOptions = {
  preLineParse: lineValue => lineValue,
  postLineParse: lineValue => lineValue,
...
function formatSource(lines, options) {
  ...
  const result = _(opts.preParse(lines))
    .map((val, lineNb) => formatLine(val, lineNb, {
 ...
 */
function getSyntax({
  controls, declarations, types, comments,
}) {
  return [
    { // Controls
